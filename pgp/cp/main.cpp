...
biff[20 * w * h]
*/
void render(vec3 pc, vec3 pv, int w, int h, double angle, Canvas::TCanvas *canvas) {
	int i, j;
	double dw = 2.0 / (w - 1.0);
	double dh = 2.0 / (h - 1.0);
	...
	for(i = 0; i < w; i++)	
		for(j = 0; j < h; j++) {
			...
			uchar4 c = ray(pc, norm(dir));
			Canvas::PutPixel(canvas, { i, h - 1 - j }, Canvas::TColor { c.x, c.y, c.z, c.w });
/*			print(pc);
			print(add(pc, dir));
			printf("\n\n\n"); */
		}
